<template>
  <div>
    <el-card>
      <svg-icon class="icon-mg-r" name="top-list"></svg-icon>
      <i class="font-title">本周热门</i>
      <div class="list-item" v-for="(item, index) in hotList" :key="index">
        <svg-icon class="icon-mg-r icon-small" name="item" />
        <el-link :underline="false" href="https://element.eleme.io">
          <i class="font-base">{{ item.title }}</i></el-link
        >
      </div>
    </el-card>

    <el-card>
      <div class="tech-item" v-for="(item, index) in techList" :key="index">
        <div class="logo">
          <img class="image" :src="item.logoUrl" />
          <h5>{{ item.name }}</h5>
          <i class="font-base">{{ item.desc }}</i>
        </div>
      </div>
    </el-card>

    <el-card>
      <svg-icon class="icon-mg-r" name="list"></svg-icon>
      <i class="font-title">友情链接</i>
      <div class="list-item" v-for="(item, index) in links" :key="index">
        <svg-icon class="icon-mg-r icon-small" name="item" />
...
export default {
  ...
  data() {
    return {
      hotList: [
        { title: "设计模式 – Singleton 单例模式", id: "" },
        { title: "dob – 框架实现", id: "" },
        {
          title: "设计模式 – Abstract Factory 抽象工厂",
          id: "",
        },
        { title: "设计模式 – Adapter 适配器模式", id: "" },
      ],
      techList: [
        {
          name: "GraphQL",
          index: "",
          logoUrl: "../../assets/picture/logo.svg",
          desc: "一种用于 API 的查询语言",
        },
        {
          name: "Vue.js",
